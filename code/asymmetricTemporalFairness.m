% problem 4: minimum assignment r_i = 1/4,1/8 for asymmetric scenario
...
time_slot = 10^4;
D = 1000;
user1 = [D/5 0];    %the coordinate of users
user2 = [0 D/4];
user3 = [-D/3 0];
user4 = [0 -D/2];
alpha = 4;
...
fprintf(" v_1=%.4f\n v_2=%.4f\n v_3=%.4f\n v_4=%.4f\n",v(1),v(2),v(3),v(4));
fprintf("The throughput of user1 is:%.4f The time fraction is:%.2f\n", throughput_user1_v,sum(flag_v==1)/time_slot);
fprintf("The throughput of user2 is:%.4f The time fraction is:%.2f\n", throughput_user2_v,sum(flag_v==2)/time_slot);
fprintf("The throughput of user3 is:%.4f The time fraction is:%.2f\n", throughput_user3_v,sum(flag_v==3)/time_slot);
fprintf("The throughput of user4 is:%.4f The time fraction is:%.2f\n", throughput_user4_v,sum(flag_v==4)/time_slot);

fprintf("when r=1/8 \n v_1=%.4f\n v_2=%.4f\n v_3=%.4f\n v_4=%.4f\n",u(1),u(2),u(3),u(4));
fprintf("The throughput of user1 is:%.4f The time fraction is:%.2f\n", throughput_user1_u,sum(flag_u==1)/time_slot);
fprintf("The throughput of user2 is:%.4f The time fraction is:%.2f\n", throughput_user2_u,sum(flag_u==2)/time_slot);
fprintf("The throughput of user3 is:%.4f The time fraction is:%.2f\n", throughput_user3_u,sum(flag_u==3)/time_slot);
fprintf("The throughput of user4 is:%.4f The time fraction is:%.2f\n", throughput_user4_u,sum(flag_u==4)/time_slot);
